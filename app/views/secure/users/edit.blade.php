	...
	<div class="swiper-container">
	  <div class="swiper-wrapper">
	      ...
	      <div class="swiper-slide"> 
	        ...
	        {{Form::model($user, ['route'=>['secure.users.update', Crypt::encrypt($user->id)], 'method' => 'PUT'])}}
				<p>
					<label>Admin</label>
					<span class="field">
				...
				<p>
					...
					<span class="field">
						{{Form::password('password',Input::old('password'),array('class'=>($errors->first('password')) ? 'i-error' : ' '))}}
			...
			<table class="access-level" style="width:100%">
				...
				<tr>
					...
					@for($i = 0; $i < 4; $i++)
						@if($i == 0)
							<td><p>
								{{Form::checkbox('chkmodule' . $module->id . '[]', $i,$module->tread)}}
							</p></td>
						@elseif($i == 1)
							<td><p>
								{{Form::checkbox('chkmodule' . $module->id . '[]', $i,$module->tcreate)}}
							</p></td>
						@elseif($i == 2)
							<td><p>
								{{Form::checkbox('chkmodule' . $module->id . '[]', $i,$module->tupdate)}}
							</p></td>
						@else
							<td><p>
								{{Form::checkbox('chkmodule' . $module->id . '[]', $i,$module->tdelete)}}
							</p></td>
						@endif
					@endfor
				</tr>
